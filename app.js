...
const openMessengerButton = document.querySelector("#openMessengerButton");
...
const resetPassengerCalculatorButton = document.querySelector("#resetPassengerCalculatorButton");
const printPassengerCalculatorButton = document.querySelector("#printPassengerCalculatorButton");
const openChangePasswordButton = document.querySelector("#openChangePasswordButton");
...
const confirmNewPasswordField = document.querySelector("#confirmNewPassword");
const changePasswordStatus = document.querySelector("#changePasswordStatus");
...
const messengerOverlay = document.querySelector("#messengerOverlay");
...
const messengerContactList = document.querySelector("#messengerContactList");
const messengerConversationTitle = document.querySelector("#messengerConversationTitle");
...
const cloudSyncPollIntervalMs = 30000;
...
const sessionUsernameKey = "officePortalUsername";
...
async function ensurePdfJsLibrary() {
  ...
  if (!pdfJsLibraryPromise) {
    pdfJsLibraryPromise = loadExternalScript("https://cdn.jsdelivr.net/npm/pdfjs-dist@4.4.168/build/pdf.min.js")
...
async function ensureTesseractLibrary() {
  ...
  if (!tesseractLibraryPromise) {
    tesseractLibraryPromise = loadExternalScript("https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js")
...
async function updateCloudAuthPassword(newPassword) {
  if (!isCloudAuthConfigured()) {
    throw new Error("Cloud authentication is not configured.");
  }
  ...
  const response = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:update?key=${encodeURIComponent(cloudAuthApiKey)}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json"
    },
...
async function refreshCloudSharedData() {
  ...
  try {
    ...
    if (tasksResult.status === "fulfilled" && Array.isArray(tasksResult.value)) {
      const nextTasks = stringifyForComparison(tasksResult.value);
      const currentTasks = localStorage.getItem(taskStorageKey) || "";

      if (nextTasks !== currentTasks) {
        localStorage.setItem(taskStorageKey, nextTasks);
        renderTaskBoard();
      }
    }

    if (meetingsResult.status === "fulfilled" && Array.isArray(meetingsResult.value)) {
      const nextMeetings = stringifyForComparison(meetingsResult.value);
      const currentMeetings = localStorage.getItem(meetingsStorageKey) || "";

      if (nextMeetings !== currentMeetings) {
        localStorage.setItem(meetingsStorageKey, nextMeetings);
    ...
    if (messengerResult.status === "fulfilled" && Array.isArray(messengerResult.value)) {
      const nextMessages = stringifyForComparison(messengerResult.value);
      const currentMessages = localStorage.getItem(messengerStorageKey) || "";

      if (nextMessages !== currentMessages) {
        localStorage.setItem(messengerStorageKey, nextMessages);
        renderMessenger();
    ...
    if (
      ...
      if (!isEditingCurrencyExchange) {
        const nextExchange = stringifyForComparison(currencyResult.value);
        const currentExchange = localStorage.getItem(currencyExchangeStorageKey) || "";

        if (nextExchange !== currentExchange) {
          localStorage.setItem(currencyExchangeStorageKey, nextExchange);
...
async function authenticateCloudSessionForAccount(account, attemptedPassword = "") {
  if (!isCloudSyncEnabled() || !cloudSecureMode) {
    return;
  }
  ...
  const passwordToUse = String(attemptedPassword || "").trim();

  if (!passwordToUse) {
    throw new Error("Missing password for cloud authentication.");
  }

  try {
    await signInCloudAuth(account.username, passwordToUse);
  } catch (error) {
    const message = String(error?.message || "");
    ...
    if (
      ...
      await ensureCloudAuthAccount(account.username, passwordToUse);
      return;
    }
...
const mactanWeatherConfig = {
  latitude: 10.3103,
  longitude: 123.9494,
  locationLabel: "Mactan-Cebu International Airport area"
};
...
function updateHeaderProfile() {
  ...
  profileName.textContent = currentStaffName;
  profileAvatar.textContent = getStaffInitials(currentStaffName);
}

function getWeatherConditionLabel(code) {
  return weatherCodeLabels[Number(code)] || "Current conditions unavailable";
}

async function updateWeatherPanel() {
  if (!weatherTemperature || !weatherCondition || !weatherWind) {
    return;
  }

  weatherCondition.textContent = "Loading current weather...";
  weatherWind.textContent = mactanWeatherConfig.locationLabel;

  try {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${encodeURIComponent(mactanWeatherConfig.latitude)}&longitude=${encodeURIComponent(mactanWeatherConfig.longitude)}&current=temperature_2m,weather_code,wind_speed_10m&timezone=Asia%2FManila`;
    ...
    const windSpeed = Number(currentWeather?.wind_speed_10m);
    const weatherCode = currentWeather?.weather_code;

    weatherTemperature.textContent = Number.isFinite(temperature) ? `${Math.round(temperature)}°C` : "--";
    weatherCondition.textContent = getWeatherConditionLabel(weatherCode);
    weatherWind.textContent = Number.isFinite(windSpeed)
      ? `${mactanWeatherConfig.locationLabel} • Wind ${Math.round(windSpeed)} km/h`
      : mactanWeatherConfig.locationLabel;
  } catch {
    weatherTemperature.textContent = "--";
    weatherCondition.textContent = "Unable to load weather right now.";
    weatherWind.textContent = mactanWeatherConfig.locationLabel;
  }
}
...
function renderMessengerContacts(messages, currentStaffName, selectedRecipient) {
  if (!messengerContactList || !messengerRecipient) {
    return;
  }

  const recipients = Array.from(messengerRecipient.options)
    .map((option) => option.value)
    .filter((value) => value && value !== currentStaffName);

  messengerContactList.innerHTML = recipients.map((recipient) => {
    const unreadCount = messages.filter((message) =>
      message.from === recipient &&
      (message.to === currentStaffName || message.to === "All Staff")
    ).length;

    return `
      <button
        type="button"
        class="messenger-contact-item ${selectedRecipient === recipient ? "is-active" : ""}"
        data-messenger-contact="${recipient}">
        <span class="messenger-contact-avatar">${recipient === "All Staff" ? "AS" : recipient.split(" ").map((part) => part[0]).join("").slice(0, 2)}</span>
        <span class="messenger-contact-copy">
          <span class="messenger-contact-name">${recipient}</span>
          <span class="messenger-contact-note">${recipient === "All Staff" ? "Broadcast conversation" : "Direct conversation"}</span>
        </span>
        ${unreadCount ? `<span class="messenger-contact-count">${unreadCount}</span>` : ""}
      </button>
    `;
  }).join("");

  messengerContactList.querySelectorAll("[data-messenger-contact]").forEach((button) => {
    button.addEventListener("click", () => {
      setMessengerRecipient(button.dataset.messengerContact);
      if (messengerInput) {
...
function renderMessenger() {
  ...
  if (messengerConversationTitle) {
    messengerConversationTitle.textContent = selectedRecipient;
  }

  renderMessengerContacts(messages, currentStaffName, selectedRecipient);
...
function openMessenger() {
  if (!messengerPanel || !messengerOverlay) {
    return;
  }

  messengerOverlay.classList.remove("workspace-overlay-hidden");
  messengerOverlay.classList.add("workspace-overlay-visible");
  messengerPanel.classList.remove("task-tracker-panel-hidden");
  messengerPanel.classList.add("task-tracker-panel-visible");
...
window.openMessengerPanel = openMessenger;
...
function closeMessenger() {
  ...
  messengerOverlay.classList.remove("workspace-overlay-visible");
  messengerOverlay.classList.add("workspace-overlay-hidden");
  messengerPanel.classList.remove("task-tracker-panel-visible");
  messengerPanel.classList.add("task-tracker-panel-hidden");
...
function renderNotifications() {
  ...
  const messengerMessages = getVisibleMessengerMessages();
  ...
  const unreadMessengerNotifications = messengerMessages
    .filter((message) =>
      message.from !== currentStaffName &&
      message.createdAt > lastReadAt &&
      (message.to === "All Staff" || message.to === currentStaffName)
  ...
  unreadMessengerNotifications.forEach((message, index) => {
    notifications.unshift({
      id: `messenger-alert-${message.id}-${index}`,
      kind: "attendance",
      classes: "attendance-item is-highlight attendance-item-reminder",
      title: "New Staff Message",
      time: `${message.from} sent you a message.`,
      note: message.to === "All Staff"
        ? `Broadcast message received at ${message.timestamp}.`
        : `Direct message received at ${message.timestamp}.`
    });
  });
...
function renderTaskBoard() {
  ...
  const currentStaffName = getCurrentStaffName();
  const canDeleteTasks = currentStaffName === "Wendixter Cutanda" || currentStaffName === "Jay A. Oyao";
...
function deleteTask(taskId) {
  ...
  if (currentStaffName !== "Wendixter Cutanda" && currentStaffName !== "Jay A. Oyao") {
    return;
  }
...
function getCurrentStaffUsername() {
  return getSessionItem(sessionUsernameKey) || "";
}
...
function handleLogin(event) {
  ...
  if (matchedAccount && (cloudSecureMode ? true : matchedAccount.password === password)) {
    authenticateCloudSessionForAccount(matchedAccount, password)
      .then(() => {
        ...
        setSessionItem(sessionUsernameKey, matchedAccount.username);
...
function handleLogout() {
  ...
  removeSessionItem(sessionUsernameKey);
...
function normalizeFirebaseAuthError(message) {
  ...
  if (normalized.includes("TOKEN_EXPIRED") || normalized.includes("CREDENTIAL_TOO_OLD_LOGIN_AGAIN")) {
...
async function handleChangePassword(event) {
  ...
  if (!currentPasswordField || !newPasswordField || !confirmNewPasswordField || !changePasswordStatus) {
    return;
  }

  const username = getCurrentStaffUsername();
  const currentPassword = currentPasswordField.value.trim();
  const newPassword = newPasswordField.value.trim();
  const confirmPassword = confirmNewPasswordField.value.trim();
  const matchedAccount = staffAccounts.find((account) => account.username === username);

  changePasswordStatus.classList.remove("is-warning", "is-ready");

  if (!matchedAccount) {
    changePasswordStatus.textContent = "Unable to identify the signed-in staff account.";
    changePasswordStatus.classList.add("is-warning");
    return;
  }
  ...
  if (currentPassword === newPassword) {
    changePasswordStatus.textContent = "Use a new password that is different from your current one.";
    changePasswordStatus.classList.add("is-warning");
    return;
  }

  if (!cloudSecureMode || !isCloudSyncEnabled() || !isCloudAuthConfigured()) {
    changePasswordStatus.textContent = "Password change requires the Firebase secure sign-in setup.";
    changePasswordStatus.classList.add("is-warning");
    return;
  }
  ...
  try {
    await signInCloudAuth(username, currentPassword);
    await updateCloudAuthPassword(newPassword);
    changePasswordStatus.textContent = "Password updated successfully. Use the new password on your next sign-in.";
    changePasswordStatus.classList.add("is-ready");

    if (changePasswordForm) {
      changePasswordForm.reset();
    }
  } catch (error) {
    changePasswordStatus.textContent = normalizeFirebaseAuthError(error?.message);
    changePasswordStatus.classList.add("is-warning");
  }
}
...
function openChangePassword() {
  if (!changePasswordOverlay || !changePasswordPanel) {
    return;
  }
  ...
  changePasswordOverlay.classList.remove("workspace-overlay-hidden");
  changePasswordOverlay.classList.add("workspace-overlay-visible");
  changePasswordPanel.classList.remove("task-tracker-panel-hidden");
  changePasswordPanel.classList.add("task-tracker-panel-visible");
  document.body.classList.add("workspace-open");
...
window.openChangePasswordPanel = openChangePassword;
...
function closeChangePassword() {
  ...
  changePasswordOverlay.classList.remove("workspace-overlay-visible");
  changePasswordOverlay.classList.add("workspace-overlay-hidden");
  changePasswordPanel.classList.remove("task-tracker-panel-visible");
  changePasswordPanel.classList.add("task-tracker-panel-hidden");
  document.body.classList.remove("workspace-open");
}
...
function resetPassengerCalculator() {
  if (passengerCalculatorForm) {
    passengerCalculatorForm.reset();
  }

  if (calculatorExchangeRate) calculatorExchangeRate.value = "";
  if (calculatorDutiableValue) calculatorDutiableValue.textContent = "--";
  if (calculatorCustomsDuties) calculatorCustomsDuties.textContent = "--";
  if (calculatorCdsDst) calculatorCdsDst.textContent = "130";
  if (calculatorImportProcessingCharge) calculatorImportProcessingCharge.textContent = "--";
  if (calculatorLandedCost) calculatorLandedCost.textContent = "--";
  if (calculatorExciseTaxDisplay) calculatorExciseTaxDisplay.textContent = "--";
  if (calculatorVat) calculatorVat.textContent = "--";
  if (calculatorSummaryCustomsDuties) calculatorSummaryCustomsDuties.textContent = "--";
  if (calculatorSummaryVat) calculatorSummaryVat.textContent = "--";
  if (calculatorSummaryImportProcessingCharge) calculatorSummaryImportProcessingCharge.textContent = "--";
  if (calculatorSummaryCdsDst) calculatorSummaryCdsDst.textContent = "130";
  if (calculatorSummaryExciseTax) calculatorSummaryExciseTax.textContent = "--";
  if (calculatorSummarySurcharge) calculatorSummarySurcharge.textContent = "--";
  if (calculatorAmountDue) calculatorAmountDue.textContent = "--";
  if (calculatorSummary) calculatorSummary.textContent = "Enter passenger details and calculate.";
  if (printPassengerName) printPassengerName.textContent = "--";
  if (printFlightNumber) printFlightNumber.textContent = "--";
  if (printCommodity) printCommodity.textContent = "--";
  if (printCurrencyUsed) printCurrencyUsed.textContent = "--";
  if (printCommodityValue) printCommodityValue.textContent = "--";
  if (printExchangeRate) printExchangeRate.textContent = "--";
  if (printDutyRate) printDutyRate.textContent = "--";
  if (printSurchargeStatus) printSurchargeStatus.textContent = "Not Applicable";
  if (calculatorPrintTimestamp) calculatorPrintTimestamp.textContent = "Generated for printing";
}
...
[
  ...
  ["updateWeatherPanel", updateWeatherPanel],
...
if (resetPassengerCalculatorButton) {
  resetPassengerCalculatorButton.addEventListener("click", resetPassengerCalculator);
}
...
if (openChangePasswordButton) {
  openChangePasswordButton.addEventListener("click", openChangePassword);
}

if (changePasswordOverlay) {
  changePasswordOverlay.addEventListener("click", closeChangePassword);
}
...
if (messengerOverlay) {
  messengerOverlay.addEventListener("click", closeMessenger);
}
...
setInterval(enforceCloudSessionHealth, 60 * 1000);
